configuration DuCUG_XenDesktop76 {
    ...
    Import-DscResource -ModuleName PSDesiredStateConfiguration, CitrixXenDesktop7Lab, cPfxCertificate;
...
#region XenDesktop Controllers
    ...
    node ($AllNodes | Where Role -eq 'Controller').NodeName {
    
        File StarLabLocalPfx {
            DestinationPath = "$env:SystemDrive\BootStrap\star.lab.local.pfx";
        ...
        cPfxCertificate StarLabLocal {
            ...
            Store = 'My';
            Path = "$env:SystemDrive\BootStrap\star.lab.local.pfx";
            Exportable = $true;
            Credential = $PfxCertificateCredential;
            DependsOn = '[File]StarLabLocalPfx';
        }

    } #end node All XenDesktop Controllers
    ...
    } #end node First XenDesktop Controller
    ...
    } #end node Additional XenDesktop Controllers
